...
= 1.0.5.55 =
* **Migration:** Admin notice when Geo Core is active; defers Elementor element controls to Geo Core native integration (`rwgc_elementor_native_elements_active`); legacy `egp_is_pro_user` maps to GeoCore Pro when advanced targeting is enabled.
...
= 1.0.5.52 =
* **Admin IA:** Elementor under Integrations; Geo Content and Dynamic Content under Experiences; city settings under Integrations (hidden nav); groups/time addons hidden from primary section nav.

= 1.0.5.51 =
* **Phase 2:** Platform shell page headers on Geo Elementor admin screens; geo rule edit uses visibility rule builder copy and mount wrapper.

= 1.0.5.50 =
* **Platform:** Shell-only hub route registration (`register_wp_submenu` false); skip legacy inner nav when platform shell is active.

= 1.0.5.49 =
* **Fix:** Register hub submenus at `admin_menu` priority 20 (after Geo Core parent at priority 5) so `?page=geo-elementor` and related targeting screens are allowed in wp-admin again.
...
= 1.0.5.39 =
* **Geo rules (front end):** Rules with target type **popup** are no longer skipped; numeric targets are treated as **Elementor popup template post IDs** and matched to **`data-elementor-lightbox`** triggers and **`elementor-action`** popup links (in addition to **`data-id`** for sections/widgets).
* **Note:** Saving a **section** rule with a popup template ID (e.g. `12354`) was never a valid `data-id`; use **popup** type or pick the real section/container **data-id** from the builder.

= 1.0.5.38 =
* Popup geo rules (library popups): attach to **Elementor Pro** `elementorProFrontend.modules.popup` first (with short retry while Pro loads), then fall back to `documents[0]`. Normalizes `{ id: … }` vs numeric popup ids. Removes reliance on an empty `documents[0]` array on newer Elementor.
