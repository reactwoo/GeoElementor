...
class EGP_Geo_Section_Widget extends \Elementor\Widget_Base {
    ...
    protected function get_countries_select_html($control_id, $label) {
        ...
        $html .= '</select>';
        $html .= '<p class="description">' . esc_html__('Hold Ctrl/Cmd to select multiple countries. Leave empty to use template countries, or override them here.', 'elementor-geo-popup') . '</p>';
        $html .= '</div>';
        $html .= '</div>';
        return $html;
    }
